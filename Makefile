...
# 镜像仓库项目
DOCKER_REG_SPACE = jike
DOCKER_REG_SPACE_MZ = mzmovie
DOCKER_REG_SPACE_SDYX = jike
...
K8S_CONTEXT =
K8S_CONTEXT_DEV = dev
K8S_CONTEXT_PROD = dmz-prod
K8S_CONTEXT_STAGE = dmz-stage

# K8S命名空间
K8S_NAMESPACE =
K8S_NAMESPACE_SDYX = sdyxmall
K8S_NAMESPACE_MZ = mzmovie
...
ifeq (sdyxmall, $(NAME_SPACE))
	K8S_NAMESPACE = $(K8S_NAMESPACE_SDYX)
	DOCKER_REG_SPACE = $(DOCKER_REG_SPACE_SDYX)
else ifeq (mzmovie, $(NAME_SPACE) )
    K8S_NAMESPACE = $(K8S_NAMESPACE_MZ)
    DOCKER_REG_SPACE = $(DOCKER_REG_SPACE_MZ)
else
...
define vv
kubectl describe po $(SERVICE_NAME) | grep Image:
endef
...
config: init
	...
	kubectl apply -f tmp/$(DEPLOY_ENV)/configmap.yaml
...
k8s:
	...
	kubectl apply -f tmp/$(DEPLOY_ENV)/deployment_$(DOCKER_TAG).yaml
	kubectl get po | grep $(SERVICE_NAME)

# ----------------------------------------
# 完整发布
.PHONY : deploy
deploy: config package check docker k8s
